...
class ModelCatalogProduct extends Model {
	...
	public function editProduct($product_id, $data) {
		...
		if($data['usd'])
			$data['price'] = $data['usd'] * $currency['usd'];
		if($data['eur'])
			$data['price'] = $data['eur'] * $currency['eur'];
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_record WHERE product_id = '" . (int)$product_id . "'");
		
		if (isset($data['record_related'])) {
			foreach ($data['record_related'] as $related_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_record WHERE product_id = '" . (int)$product_id . "' AND record_id = '" . (int)$related_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_record SET product_id = '" . (int)$product_id . "', record_id = '" . (int)$related_id . "'");				
			}
		}		
	...
	public function getRecordRelated($product_id) {
		$product_record_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_record WHERE product_id = '" . (int)$product_id . "'");

		foreach ($query->rows as $result) {
			$product_record_data[] = $result['record_id'];
		}
	
		return $product_record_data;
	}
