...
class ModelCatalogFields extends Model
{
	...
	{
		$sql   = "SELECT *
		FROM  " . DB_PREFIX . "review_fields rf
		 ...
		 foreach ($fields as $num => $field) {
            foreach ($desc as $num_desc => $field_desc) {
          	if ($field['field_id'] == $field_desc['field_id']) {
                 ...
                 $fields_new[$num]['field'][$field_desc['language_id']] = unserialize(base64_decode($field_desc['field'])) ;
	...
	public function getFieldByName($field_name)
	{
		...
		FROM " . DB_PREFIX . "fields f
		LEFT JOIN " . DB_PREFIX . "fields_description fd ON (f.field_id = fd.field_id)
		WHERE f.field_name = '" .  $this->db->escape(strip_tags($field_name))  . "' AND fd.language_id = '" . (int) $this->config->get('config_language_id') . "'");
		return $query->row;
	}
	...
	public function editField($field_id, $data)
	{
       ...
       $data_info = $this->getFieldOnly($field_id);
		...
		field_public = '" . (int) $data['field_public'] . "',
