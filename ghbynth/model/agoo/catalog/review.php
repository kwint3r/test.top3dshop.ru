       ...
       if (strtolower($name) =='editreview') {		  $generallist = $this->config->get('generallist');
		  if (isset($generallist['review_visual']) && $generallist['review_visual']) {
			 if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateFormReview()) {
				...
				$product_id = $this->model_catalog_blogreview->getProductIdbyReviewId($this->request->get['review_id']);
	            ...
	            if (isset($this->request->post['notify']) && $this->request->post['notify']) {
	                    ...
	                    $product_info['comment_id']								  = $this->request->get['review_id'];
