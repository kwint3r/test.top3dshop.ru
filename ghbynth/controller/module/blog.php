...
/***************************************/
	public function install_admin_loader()
	{
		$html = '';
...
/***************************************/
	private function remove_admin_loader()
	{
		$html = "<br>";
		$file = DIR_APPLICATION . 'controller/common/home.php';
		if (file_exists($file)) {
...
/***************************************/
	public function mylist_save()
	{
 	  $this->load->model('setting/setting');

 	//  $file = DIR_CACHE . 'mylist_save.lock.'.$this->request->get['index'];
 	//  $file_next = DIR_CACHE . 'mylist_save.lock.'.((int)$this->request->get['index']+1);

      //if ($this->request->get['index']=="0") {
      // touch($file);
     // }
  	 ...
  	 // }
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
		 ...
		 $this->data['token'] = $this->session->data['token'];
		  ...
		  if (isset($this->request->post['mylist'])) {
		    $mylist_post = $this->request->post['mylist'];
		  } else {
		  	$mylist_post = Array();
		  }
...
//print_r("<PRE>");
//echo  key($mylist_post)."->".$mylist_post[key($mylist_post)]['remove'];
//print_r($modules);
//print_r("</PRE>");



         $mylist[key($mylist_post)] = $mylist_post[key($mylist_post)];


         if (isset($mylist_post[key($mylist_post)]['remove']) && $mylist_post[key($mylist_post)]['remove']=="remove") {
           unset($mylist[key($mylist_post)]);

           foreach ($modules as $num => $value) {
           	if ($value['what'] == key($mylist_post)) {
           	 //echo "delete";
           	 unset($modules[$num]);
           	 $modules_new['blog_module'] = $modules;
           	 $this->model_setting_setting->editSetting('blog_schemes', $modules_new);
           	}
	     ...
	     $mylist_new['mylist'] = $mylist;

		 $this->model_setting_setting->editSetting('blog_widgets', $mylist_new);
	...
	public function ajax_list()
	{
		...
		if ($this->data['mylist'][$num]['type'] == 'related') {
			...
			$this->template = 'module/blog_list.tpl';
		}
		...
		if (!isset($this->data['mylist'][$num]['anchor']) && $this->data['mylist'][$num]['type'] == 'treecomments') {
			$this->data['mylist'][$num]['anchor'] = "\$('#tab-review').html(data);";
		}
		...
		if (!isset($this->data['mylist'][$num]['description_status'])) {
			$this->data['mylist'][$num]['description_status'] = true;
		}
		if (!isset($this->data['mylist'][$num]['avatar_status'])) {
			$this->data['mylist'][$num]['avatar_status'] = true;
		}

		if (!isset($this->data['mylist'][$num]['cached'])) {
			$this->data['mylist'][$num]['cached'] = true;
		}
		if (!isset($this->data['mylist'][$num]['images_view'])) {
			$this->data['mylist'][$num]['images_view'] = true;
		}

		if (!isset($this->data['mylist'][$num]['view_comments'])) {
			$this->data['mylist'][$num]['view_comments'] = true;
		}
		if (!isset($this->data['mylist'][$num]['karma'])) {
			$this->data['mylist'][$num]['karma'] = true;
		}

		if (!isset($this->data['mylist'][$num]['fields_view'])) {
			$this->data['mylist'][$num]['fields_view'] = true;
		}
		if (!isset($this->data['mylist'][$num]['view_captcha'])) {
			$this->data['mylist'][$num]['view_captcha'] = true;
		}
		if (!isset($this->data['mylist'][$num]['status_now'])) {
			$this->data['mylist'][$num]['status_now'] = true;
		}

		if (!isset($this->data['mylist'][$num]['view_category'])) {
			$this->data['mylist'][$num]['view_category'] = true;
		}

		if (!isset($this->data['mylist'][$num]['view_record'])) {
			$this->data['mylist'][$num]['view_record'] = true;
		}

		if (!isset($this->data['mylist'][$num]['view_avatar'])) {
			$this->data['mylist'][$num]['view_avatar'] = true;
		}
		if (!isset($this->data['mylist'][$num]['view_author'])) {
			$this->data['mylist'][$num]['view_author'] = true;
		}
		if (!isset($this->data['mylist'][$num]['field_public'])) {
			$this->data['mylist'][$num]['field_public'] = true;
		}
