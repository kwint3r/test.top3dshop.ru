...
class ControllerFeedYandexYml extends Controller {
	...
	protected function setFormData() {
		...
		*/
		...
		if (isset($this->request->post['yandex_yml_in_stock'])) {
			...
			$this->data['yandex_yml_in_stock'] = $this->config->get($this->CONFIG_PREFIX.'in_stock');
		} else {
		...
		if (isset($this->request->post['yandex_yml_out_of_stock'])) {
			...
			$this->data['yandex_yml_out_of_stock'] = $this->config->get($this->CONFIG_PREFIX.'out_of_stock');
		} else {
	...
	public function autocomplete() {
		...
		if (count($parts) < 2) {
			$q = $this->db->query("SELECT DISTINCT CONCAT(level1) AS text FROM " . DB_PREFIX . "yandex_category WHERE level1 LIKE '".$this->db->escape($parts[0])."%' AND level2='' AND level3=''");
		}
		elseif (count($parts) < 3) {
			$q = $this->db->query("SELECT DISTINCT CONCAT(level1,'/',level2) AS text FROM " . DB_PREFIX . "yandex_category WHERE level1='".$this->db->escape($parts[0])."' AND level2 LIKE '".$this->db->escape($parts[1])."%' AND level3=''");
		}
		elseif (count($parts) < 4) {
			$q = $this->db->query("SELECT DISTINCT CONCAT(level1,'/',level2,'/',level3) AS text FROM " . DB_PREFIX . "yandex_category WHERE level1='".$this->db->escape($parts[0])."' AND level2='".$this->db->escape($parts[1])."' AND level3 LIKE '".$this->db->escape($parts[2])."%'");
		}
		elseif (count($parts) < 5) {
			$q = $this->db->query("SELECT DISTINCT CONCAT(level1,'/',level2,'/',level3,'/',level4) AS text FROM " . DB_PREFIX . "yandex_category WHERE level1='".$this->db->escape($parts[0])."' AND level2='".$this->db->escape($parts[1])."' AND level3='".$this->db->escape($parts[2])."' AND level4 LIKE '".$this->db->escape($parts[3])."%'");
		}
		else {
			$q = $this->db->query("SELECT DISTINCT CONCAT(level1,'/',level2,'/',level3,'/',level4,'/',level5) AS text FROM " . DB_PREFIX . "yandex_category WHERE level1='".$this->db->escape($parts[0])."' AND level2='".$this->db->escape($parts[1])."' AND level3='".$this->db->escape($parts[2])."' AND level4='".$this->db->escape($parts[3])."' AND level5 LIKE '".$this->db->escape($parts[4])."%'");
		}
		$ret = array();
