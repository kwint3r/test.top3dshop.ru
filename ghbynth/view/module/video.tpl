      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <table id="module" class="list">
          ...
          <tbody id="module-row<?php echo $module_row; ?>">
            <tr>
              <td class="left"><select name="video_module[<?php echo $module_row; ?>][video_id]">
                  <?php foreach ($videos as $video) { ?>
                  <?php if ($video['video_id'] == $module['video_id']) { ?>
                  <option value="<?php echo $video['video_id']; ?>" selected="selected"><?php echo $video['name']; ?></option>
                  <?php } else { ?>
                  <option value="<?php echo $video['video_id']; ?>"><?php echo $video['name']; ?></option>
                  <?php } ?>
                  <?php } ?>
                </select></td>
              <td class="left"><input type="text" name="video_module[<?php echo $module_row; ?>][width]" value="<?php echo $module['width']; ?>" size="3" />
                <input type="text" name="video_module[<?php echo $module_row; ?>][height]" value="<?php echo $module['height']; ?>" size="3" />
...
function addModule() {	
	...
	html += '  <tr>';
	html += '    <td class="left"><select name="video_module[' + module_row + '][video_id]">';
	<?php foreach ($videos as $video) { ?>
	html += '      <option value="<?php echo $video['video_id']; ?>"><?php echo addslashes($video['name']); ?></option>';
	<?php } ?>
	html += '    </select></td>';
	html += '    <td class="left"><input type="text" name="video_module[' + module_row + '][width]" value="" size="3" /> <input type="text" name="video_module[' + module_row + '][height]" value="" size="3" /></td>'; 
