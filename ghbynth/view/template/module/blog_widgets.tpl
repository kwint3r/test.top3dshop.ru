...
form_submit_new = function() {
    ...
    var seq = new Array();
    for (i=0; i<$('.mylist_form').length; i++) {
        seq.push(i);
    }
    ...
    function go(){
        ...
        if(seq.length) {
          ...
          kf = $('.mylist_form')[num];
            $.ajax({
                url: 'index.php?route=module/blog/mylist_save&num='+num+'&token=<?php echo $token; ?>',
                type: 'post',
					data: $(kf).serialize(),
					dataType: 'html',
					beforeSend: function() {
                	...
                	complete: function(){
					loading_recent = Math.round((100*($('.mylist_form').length - seq.length))/$('.mylist_form').length);
...
<script type="text/javascript">
	 ...
	 form_submit_new_ = function() {
		...
		$('.mylist_form').each(function(index, value) {

		$.ajax({
					url: 'index.php?route=module/blog/mylist_save&index='+index+'&all='+$('.mylist_form').length+'&token=<?php echo $token; ?>',
					type: 'post',
					async: true,
					data: $(this).serialize(),
					dataType: 'html',
