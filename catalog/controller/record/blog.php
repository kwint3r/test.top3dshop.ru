...
class ControllerRecordBlog extends Controller
{
	...
	public function index()
	{
		...
		if (file_exists('catalog/view/theme/' . $this->config->get('config_template') . '/stylesheet/blog.css')) {
			...
			if (file_exists('catalog/view/theme/default/stylesheet/blog.css')) {
				...
				if (file_exists(DIR_CACHE. $css_name)) {
					$this->document->addStyle($this->getCacheDir().$css_name);
				} else {
		...
		if ($blog_info) {
			...
			foreach ($results as $result) {
				...
				if (!isset($dimensions['height']) || $dimensions['height'] == '')
					...
					$image                   = $this->model_tool_image->resize($result['image'],500,350);
					$this->data['image_dim'] = $dimensions;
				} //$result['image']
				else {
					$image                   = false;
			...
			if ($result['description']) {
				...
				$this->data['records'][] = array(
					...
					'description_full'=> utf8_substr(strip_tags(html_entity_decode($result['description_full'], ENT_QUOTES, 'UTF-8')), 0, 400) . '..',
	...
	{
		$array_dir_cache = str_split(DIR_CACHE);
		$array_dir_app   = str_split(DIR_APPLICATION);
		$i               = 0;
		$dir_root        = '';
		...
		$dir_cache = str_replace($dir_root, '', DIR_CACHE);
