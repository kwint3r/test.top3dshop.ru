...
class ControllerCheckoutCart extends Controller {
	...
	public function add() {
		...
		$this->model_account_customer->resetTimeLastNotify($this->customer->getId());

		$product_info = $this->model_catalog_product->getProduct($product_id);
		
	/* ADD OCDESIGN.RU VKREDIT PAYMENT */
		if(isset($this->request->post['vkredit']) && $this->request->post['vkredit'] == '1'){
			$this->session->data['vkredit'] = $this->request->post['vkredit'];
		}else{
			unset($this->session->data['vkredit']);
		}
		$this->session->data['vkredit'] = 1;
	/* END OCDESIGN.RU VKREDIT PAYMENT */
