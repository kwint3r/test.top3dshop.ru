...
class ControllerCheckoutSimpleCheckout extends Controller { 
	...
	/* ADD OCDESIG.RU KLADR */
		$this->document->addScript('catalog/view/javascript/kladr/jquery.kladr.min.js');
		$this->document->addStyle('catalog/view/javascript/kladr/jquery.kladr.min.css');
	/* END OCDESIG.RU KLADR */
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            if ($this->data['simple_steps']) {
                ...
                } elseif (!empty($this->session->data['simple_show_next_step'])) {
                    $show_next_step = 'simplecheckout_customer';
                    $tmp = trim($simple_common_template, '{');
                    $tmp = trim($tmp, '}');
                    $tmp = explode('}{', $tmp);
                    $set_next_after_customer = false;
                    foreach ($tmp as $value) {
                        if ($set_next_after_customer) {
                            $show_next_step =  'simplecheckout_'.$value;
                            break;
                        }
                        if ($value == 'customer') {
                            $set_next_after_customer = true;
                        }
                    }
                    $this->data['simple_step'] = $show_next_step;
                    unset($this->session->data['simple_show_next_step']);
                } else {
                    $this->data['simple_step'] = '';
                }
            }
            ...
            $this->data['simple_customer_hide_if_logged'] = $this->customer->isLogged() && empty($errors['customer']) ? $this->config->get('simple_customer_hide_if_logged') && $this->config->get('simple_address_hide_if_logged') : false;
