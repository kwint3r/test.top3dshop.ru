...
class ControllerProductProduct extends Controller {
	...
	/* ADD OCDESIGN.RU Вывод данных information в категории товаров */
        $this->load->model('catalog/category');
        $product_categorys = $this->model_catalog_product->getCategories($product_id);
        ...
        foreach ($product_categorys as $product_category) {
			...
			if ($category_info) {
                $this->data['lizing_id'] = $category_info['information_id'];
				$this->data['lizing'] = $this->url->link('information/information', 'information_id=' . $category_info['information_id']);
			} else {
				$this->data['lizing_id'] = 0;
				$this->data['lizing'] = $this->url->link('information/information', 'information_id=0');
			}
			
			//$this->data['lizing_no_found'] = $category_info;		
        }
	/* END */
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$this->data['price'] = $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
       			...
       			$this->data['price_digit'] = $product_info['price'];
			} else {
				...
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') { 
					...
					foreach ($option['option_value'] as $option_value) {
						if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
							...
							$sum_price = $this->currency->format($this->tax->calculate($sum_price, $product_info['tax_class_id'], $this->config->get('config_tax')));
			...
			foreach ($results as $result) {
                ...
                /* ADD OCDESIGN.RU Вывод данных information в категории товаров */
                $this->load->model('catalog/category');
                $product_categorys = $this->model_catalog_product->getCategories($result['product_id']);

				$categories = $this->model_catalog_product->getCategories($result['product_id']);
				if ($categories){
					$categories_info = $this->model_catalog_category->getCategory($categories[0]['category_id']);
					$parent_cat_id = $categories_info['category_id'];
					$parent_cat_name = $categories_info['name'];
				}
                ...
                foreach ($product_categorys as $product_category) {
                    ...
                    if ($category_info) {
                        ...
                        $information_id = 0;
                        $informations = $this->url->link('information/information', 'information_id=0');
			...
			$results = $this->model_catalog_product->getRecordRelated($this->request->get['product_id']);
			
			$this->load->model('catalog/record');
			
			if(isset($results[0])){
				$record_info = $this->model_catalog_record->getRecord($results[0]);
