...
class ModelCheckoutOrder extends Model {
    ...
    //begin of SafeOnlineReputation block
    include_once "safeonlinerep_request.php";
    
    $params = Array
    (
     'ident' => 'top3dshop',
     'customer_email' => $data['email'],
     'customer_fullname' => trim( $data['firstname'] . ' ' . $data['lastname'] ),
     'customer_id' => $data['customer_id'],
     'customer_country' => $data['shipping_country'],
     'customer_city' => $data['shipping_city'],
     'order_id' => $order_id
    );
    
    $sr = new SafeOnlineRep_Request( $params, 'CDK7UUE1MZN9H3E6JT3QAZKQCXSB9B8P' );
    //$products = $this->model_sale_order->getOrderProducts( $this->request->get['order_id'] );
    foreach( $data['products'] as $p )
    {
      $sr->addProduct( Array( 'id' => $p['product_id'], 'name' => $p['name'] ) );
    }
    $ret = $sr->Execute();
    //end of SafeOnlineReputation block
	...
	public function confirm($order_id, $order_status_id, $comment = '', $name = '', $filename = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_check SET order_id = '" . (int)$order_id . "', name = '" . $this->db->escape($name) . "', filename = '" . $this->db->escape($filename) . "'");
			...
			if (!$this->model_payment_amazon_checkout->isAmazonOrder($order_info['order_id'])) {
				...
				/***************************************/
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_check WHERE  order_id = '" . (int)$order_id . "'");
				
				$payment_download = $query->row;
				/***************************************/
				...
				if (isset($payment_download['filename'])) 
					$mail->addAttachment(DIR_DOWNLOAD . $payment_download['filename']);
				$mail->send();
			}
