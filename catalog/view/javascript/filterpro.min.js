...
$(function () {
    $("#slider-range").slider({range:true, min:0, max:0, values:[0, 0], stop:function (a, b) {
		...
		a = numberWithCommas(b.values[0]);
		b = numberWithCommas(b.values[1]);
...
function success(g, b, append) {
    ...
    if (typeof(g.min_price) === 'string') {
	    var d = parseInt(g.min_price.trim());
	    var c = Math.ceil(parseFloat(g.max_price.trim()));
    } else {
    ...
    if (b) {
        ...
        if (typeof(max) === 'string') {
	        ...
	        if (max >= 1) {
	            d = parseInt($("#min_price").val().replace(/\s+/g, ''));
	            c = parseInt($("#max_price").val().replace(/\s+/g, ''))
	        }
        } else {
	        if (max >= 1) {
...
function getCont(){
    ...
    var cont = ".product-{view}".replace('{view}', view);
    if (!$(cont).exists()){
...
function doFilter(getPrice, getTotals, append) {
	...
	$("#filterpro #max_price").val( parseInt( $("#filterpro #max_price").val().replace(/\s+/g, '') ) );
	$("#filterpro #min_price").val( parseInt( $("#filterpro #min_price").val().replace(/\s+/g, '') ) );
    ...
    $("#filterpro #max_price").val( numberWithCommas( $("#filterpro #max_price").val() ) );
    $("#filterpro #min_price").val( numberWithCommas( $("#filterpro #min_price").val() ) );
