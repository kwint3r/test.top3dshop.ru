...
$(document).ready(function(){
	...
	$('#dellin.dellin, label[for="dellin.dellin"]').click(function() {
		$("html, body").animate({ scrollTop: $(window).height() - $(window).height() * 30 / 100 }, 1000);
		$('input#checkout_customer_main_city').focus().css('border', '1px solid red');
	});
	
	if (!$.cookie('<?php echo md5($_SERVER["HTTP_HOST"]); ?>')) {
		//$.colorbox({href:"#orderMessage",  inline:true});
        $('#nopayonline').click();
		$.cookie('<?php echo md5($_SERVER["HTTP_HOST"]); ?>', '1', { expires: 1});
		}
});
...
<div style='display:none'>
		<div id="orderMessage" style='padding:30px; background:#fff;text-align: center'>
			<p>Оплата через банковские карты возможна только после авторизации.</p>
			<button class="button btn" onclick="$.colorbox.close();">Продолжить</button>
		</div>
</div>
<?php /* END OCDESIGN.RU */ ?>
