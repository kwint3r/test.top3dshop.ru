		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					height          : 33,                 // The height of the browse button
	...
	var handlers = {
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			switch(errorCode) {
				...
				case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:
					errorString = 'Не удалось загрузить URL';
					break;
				case SWFUpload.UPLOAD_ERROR.IO_ERROR:
					errorString = 'Ошибка ввода/вывода';
					break;
				case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
					errorString = 'Ошибка безопастности';
					break;
				case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
					alert('Превышен макс. размер загружаемого файла (' + errorMsg + ').');
					errorString = 'Превышен макс. размер загружаемого файла';
					break;
				...
				case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
					errorString = 'Данный формат файла не может быть загружен';
					break;
		...
		onUploadSuccess : function(file, data, response) {
			...
			if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
				$('#' + file.id).find('.data').html(' - Файл загружен. Отправьте форму или загрузите другой файл. ');
			}
