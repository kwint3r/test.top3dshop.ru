...
<?php include('catalog/view/theme/'.$this->config->get('config_template').'/template/new_elements/wrapper_top.tpl'); ?>
  ...
  <table class="compare-info">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?php if ($products[$product['product_id']]['thumb']) { ?>
          <img id="image_<?php echo $products[$product['product_id']]['product_id']; ?>" src="<?php echo $products[$product['product_id']]['thumb']; ?>" alt="<?php echo $products[$product['product_id']]['name']; ?>" />
          <?php } ?></td>
        <?php } ?>
      </tr>
      <tr>
    ...
    <tr>
      ...
      <?php foreach ($products as $product) { ?>
      <td>
	  <?php if ($product['price']>0) {?><input type="button" value="<?php echo $button_cart; ?>" onclick="addToCart('<?php echo $product['product_id']; ?>', 1, 'image_<?php echo $products[$product['product_id']]['product_id']; ?>')" class="button" /><?php } ?>
	  </td>
      <?php } ?>
    </tr>
    <tr>
