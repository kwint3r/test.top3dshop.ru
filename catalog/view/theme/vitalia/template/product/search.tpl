...
<?php include('catalog/view/theme/'.$this->config->get('config_template').'/template/new_elements/wrapper_top.tpl'); ?>
  ...
  <div style="margin-bottom: 20px;margin-top: 0px"><?php echo $text_critea; ?></div>
 ...
 <div style="margin-bottom: 20px"><?php echo $text_search; ?></div>
	...
	<div>
		<div class="row">
			...
			<?php /* ADD OCDESIGN.RU VKREDIT PAYMENT */ ?>
				...
				<?php if ($product['lizing']) { ?>
				<?php if ($product['lizing']['link_id'] == 0) { ?>
						<input type="button" class="button-vkredit" onclick="addToVkredit('<?php echo $product['product_id']; ?>');" value="Купить в кредит" />
					<?php } else { ?>
						<input type="button" class="button-vkredit" onclick="window.location.href = '<?php echo $product['lizing']['link']; ?>';" value="Купить в лизинг" />
					<?php } ?>
				<?php } ?>
					</div>
